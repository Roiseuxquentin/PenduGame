...
// Next step => Make this with react
...
const addTime = () => {
  ...
  } else if (i === 11) { canvas.style.background = 'white' } else { canvas.style.background = 'black' }
...
const helpDeclencheur = (route) => {
  ...
  help1.addEventListener('click', event => {
    route++
    console.log('push')
...
const draw = () => {
  if (pub[pub.length - 1] === undefined) { delete pub[pub.length - 1] }

  if (dead === false) {
    document.getElementById('keyboard').addEventListener('click', klick)
...
const drawText = (text) => {
  ...
  if (i === 10) { ctx.fillStyle = 'white' } else { ctx.fillStyle = 'black' }
...
const drawPub = () => {
  ...
  if (posPub <= -500) {
    posPub = 400
    publi++
  } else if (publi > pub.length - 1) { publi = 1 } else if (dead === true) {
    ctxPub.beginPath()
    ctxPub.font = '22px Courier'
    ctxPub.fillStyle = 'white'
    ctxPub.textAlign = 'center'
    ctxPub.fillText(`${word.name.toUpperCase()}`, 200, 20)
    ctxPub.closePath()
  } else {
    posPub -= 5
    ctxPub.beginPath()
    ctxPub.font = '20px Courier'
    if (time > 3) { ctxPub.fillStyle = 'white' } else { ctxPub.fillStyle = 'red' }
  ...
  if (solution.includes(keyName) === true) {
    solution.map((element) => {
      if (element === keyName) {
        found.push(keyName)
        array.push(keyName)
      } else if (array.includes(element)) { found.push(element) } else {
        // fonction RATER////////////////////////////////////////////////////////
